...
class DataCenter(Topo):
	...
	# BUILDING THE MID LEVEL SWITCHES
	# FROM TOP LEVEL SWITCHES
	# AND ADDING SWITCH LINKS BETWEEN 
	# TLS AND MLS
	for i in range(fi):
		mid_level_switch = self.addSwitch('mls%s' % (i + 1))
		self.addLink(tls, mid_level_switch, **swlinkConfig)
		# BUILDING THE LOW LEVEL SWITCHES		
		# FROM MID LEVEL SWITCHES
		# AND ADDING SWITCH LINKS BETWEEN
		# MLS AND SLX
		for j in range(fi):
			low_level_switch = self.addSwitch('s%sx%s' %(i+1,(j + 1)))
			self.addLink(mid_level_switch, low_level_switch, **swlinkConfig)
			# BUILDING THE HOSTS RACK FOR EACH LOW LEVEL SWITCH
			# AND ADDING HOST LINKS BETWEEN 
			# LOW LEVEL SWITCH AND HOST IN EACH RACK 			
			for k in range(n):
				hosts = self.addHost('h%sx%sx%s' %(i+1,(j + 1),(k + 1)))
				self.addLink(low_level_switch, hosts, **hostlinkConfig)
